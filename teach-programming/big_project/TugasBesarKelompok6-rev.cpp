...
class OilRent {
	...
	private:
		...
		unsigned int MAX_RENT_PRICE = 150000;
...
std::vector<std::vector<OilRent::RentData>> OilRent::addRentData(std::vector<std::vector<OilRent::RentData>> mapRentData) {
	...
	// check if the company name is new or not
	unsigned int mapRow = oilMapSize[0];
	unsigned int mapCol = oilMapSize[1];

	unsigned int totalRentPrice = 0;
	for (size_t i = 0; i < mapRow; i++) {
		for (size_t j = 0; j < mapCol; j++) {
			if (mapRentData[i][j].companyName == tempCompanyName) {
				// add the totalRentPrice
				if (mapRentData[i][j].companyType == "Negara") {
					unsigned int tempPrice = OIL_PRICE_PER_POTENCY - (OIL_PRICE_PER_POTENCY * DISCOUNT_FACTOR);
					totalRentPrice += oilPotencyMap[i][j] * tempPrice;
				} else { // Swasta

					unsigned int tempPrice = OIL_PRICE_PER_POTENCY;
					totalRentPrice += oilPotencyMap[i][j] * tempPrice;
	...
	} else if (totalRentPrice > MAX_RENT_PRICE) {
		
		std::cout << "Penyewaan gagal! Total harga sewa perusahaan ";
		std::cout << tempCompanyName << " melebihi " << MAX_RENT_PRICE;
		std::cout << "!" << std::endl << std::endl;
